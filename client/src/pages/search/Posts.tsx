...
const Posts: React.FC<IProps> = ({ posts, searchQuery }) => {
  ...
  useDocumentTitle(`Search Posts | Socialo`);
    ...
    <div className="space-y-4">
      {posts.map(
        (post) =>
          post.author && (
            <div
              className="h-28 laptop:h-24 flex justify-start bg-white dark:bg-indigo-1100 rounded-md overflow-hidden cursor-pointer"
              key={post.id}
              ...
              <div className="flex-grow p-2 pb-4 max-w-sm">
                <h4
                  ...
                  dangerouslySetInnerHTML={{
                    __html: boldString(post.description, searchQuery),
                  }}
                ></h4>

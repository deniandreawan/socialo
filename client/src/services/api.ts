...
const foodieUrl = process.env.REACT_APP_SOCIALO_URL || "http://localhost:9000";
const foodieApiVersion = process.env.REACT_APP_SOCIALO_API_VERSION || "v1";
...
export const getComments = async (
  postID: string,
  { offset = 0, limit, skip, sort = "desc" }: IFetchParams
): Promise<any> => {
  try {
    const req = await axios({
      method: "GET",

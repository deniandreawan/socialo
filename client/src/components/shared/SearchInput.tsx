...
const SearchInput: React.FC<IProps> = (props) => {
  ...
  return (
    <div
      className={`input-wrapper relative flex flex-col items-center ${props.inputClassName}`}
    >
      <BiSearch className="text-gray-400 absolute left-3 top-3 z-50" />
      <input
      ...
      {searchInput && isVisibleSuggestion && (
        <div
          className={` bg-white dark:bg-indigo-1000 shadow-lg rounded-md w-full flex justify-center flex-col overflow-hidden ${
            props.floatingResult ? "absolute top-12" : "relative top-0"
          }`}
        >
          {!props.showNoResultMessage && !error && (
            <h6 className="p-4 text-xs border-b dark:text-white border-gray-100 dark:border-gray-800">
              Search Suggestion
            </h6>
          )}
          {isSuggesting && !error && (
            ...
            suggestions.map((user) => (
              ...
              >
                <div className="flex items-center">
                  ...
                  <h6 className="mr-10 text-sm max-w-md overflow-ellipsis overflow-hidden dark:text-white">
                    {user.username}
                  </h6>
                </div>
              </div>

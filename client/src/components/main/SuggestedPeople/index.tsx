...
const SuggestedPeople: React.FC = () => {
  ...
  return (
    <div className="w-full py-4 bg-white dark:bg-indigo-1000 rounded-md overflow-hidden">
      <div className="px-4 flex justify-between mb-4">
        <h4 className="dark:text-white">Suggested People</h4>
        <Link
          to={SUGGESTED_PEOPLE}
          className="text-xs underline dark:text-indigo-500"
        >
          See all
        </Link>
      </div>
            ...
            <div className="relative flex items-center justify-between px-4 py-2">
              <Link to={`/user/${user.username}`}>
                <div className="flex items-center">
                  ...
                  <h6 className="mr-10 text-sm overflow-ellipsis overflow-hidden dark:text-white">
                    {user.username}
                  </h6>
                </div>
              </Link>

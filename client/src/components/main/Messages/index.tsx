...
const Messages: React.FC<{ isAuth: boolean }> = ({ isAuth }) => {
  ...
  useEffect(() => {
    ...
    socket.on("newMessage", (newMessage: IMessage) => {
      setMessages((prevMessages) =>
        updateNewMessages(prevMessages, newMessage)
      );
  ...
  const updateNewMessages = (
    ...
    const updated = prevMessages.map((msg) => {
      ...
      if (
        arr.includes(newMessage.from.username) &&
        arr.includes(newMessage.to.username)
      ) {
        messageOnList = true;
        return newMessage;
      }
  ...
  return (
    <div className="relative">
      <div onClick={toggleMessages}>
        <Badge count={count}>
          <BiMessageSquareDots className="messages-toggle text-xl focus:outline-none dark:text-white" />
        </Badge>
      </div>
      {isMessagesOpen && (
